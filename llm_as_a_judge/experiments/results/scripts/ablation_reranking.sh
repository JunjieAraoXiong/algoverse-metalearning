#!/bin/bash
# Experiment: ablation_reranking
# Description: Baseline + cross-encoder reranking
# Generated: 2025-11-23T18:22:13.245689
...
echo "=========================================="
echo "Running: ablation_reranking"
echo "=========================================="
...
python src/bulk_testing.py \
    ...
    --max-tokens 512 \
    --use-llm-judge \
    2>&1 | tee "/Users/hansonxiong/Desktop/algoverse/LLM_as_a_Judge/experiments/results/ablation_reranking.log"
...
for f in "/Users/hansonxiong/Desktop/algoverse/LLM_as_a_Judge/experiments/results"/*.csv; do
    if [[ -f "$f" && ! "$f" == *"ablation_reranking"* ]]; then
        mv "$f" "/Users/hansonxiong/Desktop/algoverse/LLM_as_a_Judge/experiments/results/ablation_reranking_$(basename $f)"
    fi
done
