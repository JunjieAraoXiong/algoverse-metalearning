...
{{"score": 0.XX, "justification": "Brief explanation of the score"}}"""
    ...
    def _fallback_parse(raw: str) -> Tuple[Optional[float], str]:
        score_match = re.search(r'"score"\s*:\s*([0-9]*\.?[0-9]+)', raw)
        justification_match = re.search(r'"justification"\s*:\s*"([^"]*)"', raw)
        score = float(score_match.group(1)) if score_match else None
        justification = justification_match.group(1) if justification_match else raw[:200]
    ...
    try:
        ...
        try:
            ...
            last_error = str(e)
            score, justification = _fallback_parse(cleaned)

        if score is None:
            return None, f"Failed to parse judge response: {last_error or cleaned[:200]}"
